    ...
    for (const fabric of vicFabrics) {
        for (const size of kvadroSizes) {
            res.push({
                fabricId: fabric.id,
                sizeId: size.id,
                variantName: `products/kvadro-soft-${fabric.id}-${size.id}`,
            });
        }
    }
    return res;
...
const images: Record<string, ImageData[]> = Object.fromEntries(
    Object.entries({
        ...
        "vic-36": [],
        "vic-66": [],
        "vic-70": ["Dogs-7152 (1).jpg", "Dogs-7234.jpg", "Dogs-7239.jpg", "Dogs-7278.jpg"],
        "vic-80": [],
        "vic-88": [],
        "vic-93": [],
        "vic-96": [],
        ...
        })).concat([{ src: `/fabrics/vic/${id}.JPG`, alt: "Фото ткани из которой сделана лежанка Квадро Стронг" }]),
    ])
);
