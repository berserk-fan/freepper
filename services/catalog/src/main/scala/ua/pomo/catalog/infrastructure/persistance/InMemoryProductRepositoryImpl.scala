...
class InMemoryProductRepositoryImpl[F[_]: Sync] private (ref: Ref[F, Map[ProductId, Product]]) extends ProductRepository[F] {
  override def create(command: CreateProduct): F[ProductId] = ref.modify { map =>
    ...
    val res = Product(
      ...
      command.modelId,
      ProductDisplayName(""),
      Fabric(command.fabricId, FabricDisplayName(""), Image(ImageId(UUID.randomUUID()), ImageSrc(""), ImageAlt(""))),
      Size(command.sizeId, SizeDisplayName("")),
  ...
  override def update(command: UpdateProduct): F[Int] = ref.modify { map =>
    object update extends InMemoryUpdaterPoly[Product] {
      ...
      implicit val fabricUUID: Res[FabricUUID] = gen(_.focus(_.fabric.id))
      implicit val sizeUUID: Res[SizeUUID] = gen(_.focus(_.size.uuid))
