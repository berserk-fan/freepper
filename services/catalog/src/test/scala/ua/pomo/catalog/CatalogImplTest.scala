...
class CatalogImplTest extends AnyFunSuite with BeforeAndAfter {
  val impl = (
  ...
  test("list") {
    Generators.Model.selfLazyList.map(model =>
    ...
    impl.listModels(ListModelsRequest(parent.toNameString, 10, ""), null)
  }
}
