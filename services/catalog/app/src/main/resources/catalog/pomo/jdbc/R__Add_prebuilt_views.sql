-- CREATE VIEW images_prebuilt AS
...
--                'parameters', COALESCE(json_agg(pp.json), '[]')
--            ) as json
-- from parameter_lists pl
--          join parameters_prebuilt pp on pp.parameter_list_id = pl.id
-- group by pl.id;
--
...
--                'minimalPrice', (
--                    select COALESCE(min(COALESCE(p.promo_price, p.price)), 0)
--                    from products p
--                    where p.model_id = m.id),
--                'parameterLists', (
--                    select COALESCE(json_agg(plp.json), '[]')
