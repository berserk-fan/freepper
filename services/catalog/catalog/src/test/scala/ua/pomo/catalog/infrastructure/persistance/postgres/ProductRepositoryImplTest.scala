...
class ProductRepositoryImplTest extends DbUnitTestSuite with ParallelTestExecution with Fixtures with ForEachImpl {
  case class TestResources(
      imageListRepo: ImageListRepository[IO],
      categoryRepo: CategoryRepository[IO],
      modelRepo: ModelRepository[IO],
      postgres: ProductRepository[IO],
      impls: Seq[(String, Impl)],
      db: DbResources
  )

  override type TestResource = TestResources
  override type Impl = ProductRepository[IO]
  ...
  override protected def resource: Resource[IO, TestResource] =
    for {
      db <- Resources.dbTest
      productInMemory <- Resource.eval(ProductInMemoryRepositoryImpl[IO])
      ...
      RepositoryK(ModelRepositoryImpl(), db.xa.trans),
      RepositoryK(productPostgres, db.xa.trans),
