...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = (await Promise.all(categories
      .map(c => shopClient.getCategory({name: c}))))
      .flatMap(c => c.products.map(p => [c.id,p]))
      .map(([cId,pName]) => ({ params: { categoryId: cId, productId: pName.substring("products/".length) } }));
