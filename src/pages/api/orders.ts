...
function prepareCart(cart: Record<string, CartProduct>): any {
    const val = Object.entries(cart).map(([id,product])=> {
        const fabric = product.details.dogBed.fabrics.find(f => f.id == product.details.dogBed.fabricId);
        const prepFabric = fabric.displayName
        const size = product.details.dogBed.sizes.find(s => s.id == product.details.dogBed.sizeId);
...
function renderPrimitive(p: any) {
    switch (typeof p) {
        case "string": return `<span class="string">"${p}"</span>`
        case "number": return `<span class="number">${p}</span>`
        default:
            return `${p}`
    }
}
...
function renderJSON(obj: any, depth: number = 0, indentation: string = "  ") {
    ...
    for (let [key,value] of Object.entries(obj)) {
        retValue += "<div class='tree'>" + indentation.repeat(depth) + `<span class="key">${key}</span>:`
