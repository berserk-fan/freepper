...
export default function useErrorHandling(onComplete: () => void,
                                         maxClientRetries: number,
                                         maxServerRetries: number,
                                         retryPeriod: number): [SubmitState, (t: any) => Promise<void>, number] {
    ...
    function makeACall(axiosCall: () => Promise<Response>): Promise<void> {
        ...
        return promiseRetry(
            async (retry, retryNumber) => {
                ...
                if (
                    (newState === "CLIENT_ERROR" &&
                        retryNumber <= maxClientRetries) ||
                    (newState === "SERVER_ERROR" &&
                        retryNumber <= maxServerRetries)
                ) {
