...
export function readStoredCartState(): CartState {
  ...
  return parsed.selectedProducts ? parsed : { selectedProducts: [], total: 0, size: 0 };
}
...
export function setProductCountAction(productId: string, count: number): SET_PRODUCT_COUNT {
  return {
    type: "SET_PRODUCT_COUNT",
    productId,
    count
  ...
  switch (action.type) {
    case "SET_PRODUCT_COUNT": {
      return {
        ...
        selectedProducts: {
          ...selectedProducts,
          ...{[action.productId]: {...selectedProducts[action.productId], ...{count: action.count}}}
        }
      }
    }
    case "ADD_PRODUCT": {
    ...
    case "DELETE_PRODUCT": {
      return {
        total: total - selectedProducts[action.productId].price.price * selectedProducts[action.productId].count,
